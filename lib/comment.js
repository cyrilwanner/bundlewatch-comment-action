...
const buildTable = (inputFiles) => {
  const files = inputFiles.sort((a, b) => Math.abs(b.size - b.baseBranchSize) - Math.abs(a.size - a.baseBranchSize));
...
const buildComment = ({ status, fullResults, summary }) => {
  ...
  // changed files
  const changedFiles = fullResults.filter(result => result.size !== result.baseBranchSize);

  if (changedFiles.length > 0) {
    comment += '<details>\n';
    comment += '<summary>Changed files</summary>\n\n';
    comment += buildTable(changedFiles);
    comment += '</details>\n';
  }
